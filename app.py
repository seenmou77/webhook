...
Webhook OVH-Telegram avec gestion intelligente des clients CSV et Excel
...
Installation: pip install flask requests pandas openpyxl werkzeug
"""
...
def load_clients_from_excel(file_path_or_content):
    ...
    try:
        ...
        if isinstance(file_path_or_content, str):
            ...
            # Si c'est du contenu binaire
            df = pd.read_excel(io.BytesIO(file_path_or_content))
        
        print(f"🔍 Fichier Excel lu: {len(df)} lignes, {len(df.columns)} colonnes")
        print(f"📋 Colonnes détectées: {list(df.columns)}")
        
        # Nettoyer les noms de colonnes (supprimer espaces en début/fin)
        df.columns = df.columns.str.strip()
        
        # Mapping des colonnes Excel vers nos champs standards
        ...
        for index, row in df.iterrows():
            # Extraction et normalisation du téléphone
            ...
            if len(telephone) >= 10 and telephone.startswith('0'):
                # Traitement de l'adresse complète
                adresse_complete = str(row.get('Adresse', '')).strip()
                
                # Tentative d'extraction ville depuis l'adresse
                ville = ''
                if adresse_complete:
                    # Recherche pattern "code_postal VILLE" à la fin
                    import re
                    match = re.search(r'\d{5}\s+([A-Z\-\s]+)
...
<body>
    <div class="container">
        <div class="header">
            ...
            <p class="success">✅ Serveur Railway actif 24/7 - Bot configuré avec détection automatique des banques - Support CSV et Excel</p>
        </div>
        ...
        <div class="upload-section">
            ...
            <form action="/upload" method="post" enctype="multipart/form-data">
                <div class="info-box">
                    ...
                    <p><strong>🔥 Colonne obligatoire:</strong> <code>telephone</code> (ou tel, phone, numero, N° Mobile)</p>
                    <p><strong>🏦 Détection banque:</strong> <code>iban</code> (analysé automatiquement)</p>
                    <p><strong>✨ Colonnes supportées:</strong></p>
                    <ul style="text-align: left; max-width: 800px; margin: 0 auto;">
                        ...
                        <li><strong>Excel spécial:</strong> Nom, Prénom, Adresse, N° Mobile, Email, Date de Naissance, Age</li>
                        <li><strong>Divers:</strong> statut, situation_familiale</li>
                    </ul>
                    <p><strong>🔧 Gestion intelligente:</strong> Le système détecte automatiquement les formats CSV et Excel !</p>
                </div>
                <input type="file" name="file" accept=".csv,.xlsx,.xls" required style="margin: 10px 0;">
                <br>
                <button type="submit" class="btn btn-success">📁 Charger fichier CSV/Excel</button>
            </form>
        </div>
        ...
        <div class="info-box">
            ...
            <ol>
                <li>📂 Uploadez votre fichier CSV ou Excel avec les clients</li>
                ...
                <li>📋 Support automatique des formats Excel (.xlsx) et CSV (.csv)</li>
            </ol>
        </div>
    </div>
</body>
...
def upload_file():
    """Upload et traitement des fichiers CSV et Excel avec détection banques"""
    try:
        ...
        if file_extension in ['csv', 'xlsx', 'xls']:
            ...
            nb_clients = load_clients_from_file(file, filename)
        else:
            return jsonify({"error": "Format non supporté. Utilisez CSV (.csv) ou Excel (.xlsx, .xls)"}), 400
...
def health():
    return jsonify({
        "status": "healthy", 
        "service": "webhook-ovh-telegram-bank-detection-excel",
        ...
        "features": ["client_management", "telegram_webhook", "ovh_webhook", "bank_detection", "iban_analysis", "excel_support", "csv_support"]
    })

if __name__ == '__main__':
    port = int(os.environ.get('PORT', 5000))
    app.run(host='0.0.0.0', port=port, debug=False)
, adresse_complete.upper())
                    if match:
                ...
                client_data = {
                    ...
                    "entreprise": '',  # Pas dans le fichier Excel
                    "telephone": telephone,
                    ...
                    # Informations bancaires
                    "banque": '',  # Sera rempli par la détection
                    "swift": '',   # Pas dans le fichier Excel
                    "iban": str(row.get('IBAN', '')).strip(),
                    
                    # Informations personnelles  
                    "sexe": '',  # Pas dans le fichier Excel
                    "date_naissance": date_naissance,
                    "lieu_naissance": 'Non renseigné',  # Pas dans le fichier Excel
                    "profession": '',  # Pas dans le fichier Excel
                    "nationalite": '',  # Pas dans le fichier Excel
                    "situation_familiale": '',  # Pas dans le fichier Excel
...
def load_clients_from_file(file_obj, filename):
    """Fonction unifiée pour charger clients depuis CSV ou Excel"""
    file_extension = filename.lower().split('.')[-1]
    
    if file_extension == 'csv':
        content = file_obj.read().decode('utf-8-sig')
        return load_clients_from_csv(content)
    elif file_extension in ['xlsx', 'xls']:
        content = file_obj.read()
        return load_clients_from_excel(content)
    else:
        raise ValueError(f"Format de fichier non supporté: {file_extension}. Utilisez CSV (.csv) ou Excel (.xlsx, .xls)")
